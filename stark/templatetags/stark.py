...
def body_list(cl):
    ...
    for row in cl.queryset:
        ...
        if not cl.list_display:
            ...
            yield row_list
            continue
        # print(cl.list_display)
        for name_or_func in cl.list_display:
            if isinstance(name_or_func, FunctionType):
                val = name_or_func(cl.config, row=row)
            else:
                # 进行判断 如果是choice那么显示get_foo_display
                if  cl.config.model_class._meta.get_field(name_or_func).choices:
                    val=getattr(row, 'get_%s_display'%name_or_func)
                else:
